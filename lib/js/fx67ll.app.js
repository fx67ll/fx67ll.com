...
(function (global) {
	...
	function initBrandEasterEgg() {
		...
		// 点击品牌文字：
		// · 桌面端 —— 悬停 / 聚焦时已展示链接，直接弹出确认弹窗，确认后跳转气泡中展示的同一链接；
		// · 移动端 —— 首次点按仅展示链接气泡（等效桌面悬停），再次点按才弹出确认弹窗。
		...
		if (hoverCapable) {
			...
			// 移动端等效交互：点按品牌文字展示链接气泡，点按页面其他位置收起气泡（等效移开鼠标）
			function dismissBubbleFromOutside(event) {
				if (!bubbleVisible) return;
				if (brand.contains(event.target)) return;
				hideBubble();
			}
			document.addEventListener('touchstart', dismissBubbleFromOutside);
			document.addEventListener('click', dismissBubbleFromOutside);
		}
